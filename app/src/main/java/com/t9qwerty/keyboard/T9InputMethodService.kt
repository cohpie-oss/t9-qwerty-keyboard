...
class T9InputMethodService : InputMethodService() {
    ...
    // 0 = lower case, 1 = next character only, 2 = caps lock.
    private var capitalizationMode = 0
    private var punctuationSuggestions = false
    ...
    private val commonWordRank = mapOf("the" to 100, "to" to 98, "and" to 96, "you" to 94, "for" to 92, "with" to 90, "this" to 88, "that" to 88, "hello" to 82, "thanks" to 80, "please" to 76, "lol" to 72, "omg" to 68, "brb" to 64, "idk" to 64, "btw" to 62, "asap" to 60)
    ...
    private val commitManual = Runnable { if (pendingManual.isNotEmpty()) { currentInputConnection?.commitText(pendingManual, 1); pendingManual = ""; lastKey = null; consumeSingleCaps() } }
    ...
    override fun onStartInputView(info: EditorInfo, restarting: Boolean) { super.onStartInputView(info, restarting); val inputClass = info.inputType and InputType.TYPE_MASK_CLASS; val variation = info.inputType and InputType.TYPE_MASK_VARIATION; numericMode = inputClass == InputType.TYPE_CLASS_NUMBER || inputClass == InputType.TYPE_CLASS_PHONE || inputClass == InputType.TYPE_CLASS_DATETIME; emailMode = inputClass == InputType.TYPE_CLASS_TEXT && (variation == InputType.TYPE_TEXT_VARIATION_EMAIL_ADDRESS || variation == InputType.TYPE_TEXT_VARIATION_WEB_EMAIL_ADDRESS); setInputView(onCreateInputView()) }
    ...
    private fun groupRow(groups: List<Pair<String, Pair<Char, Float>>>, controls: Boolean = false) = LinearLayout(this).apply {
        orientation = LinearLayout.HORIZONTAL
        if (controls) addView(key(capsLabel(), 'U'), LinearLayout.LayoutParams(0, dp(29), 1.2f).apply { setMargins(dp(2), dp(2), dp(2), dp(2)) })
    ...
    private fun compactControlsRow() = LinearLayout(this).apply {
        orientation = LinearLayout.HORIZONTAL
        listOf(capsLabel() to 'U', "⌫" to '⌫').forEach { (label, action) -> addView(key(label, action), LinearLayout.LayoutParams(0, dp(30), 1f).apply { setMargins(dp(2), dp(2), dp(2), dp(2)) }) }
    }
    ...
    private fun punctuationKey() = Button(this).apply { text = "."; textSize = 14f; isAllCaps = false; minHeight = 0; minimumHeight = 0; setPadding(0, 0, 0, 0); setOnClickListener { punctuationSuggestions = true; standardPrediction = emptyList(); wordSuggestions = emptyList(); refresh() }; setOnLongClickListener { punctuationMode = true; setInputView(onCreateInputView()); true }; styleButton(this) }
    private fun capsLabel() = when (capitalizationMode) { 1 -> "⇧1"; 2 -> "⇧⇧"; else -> "⇧" }
    private fun groupKey(letters: String, action: Char, numberOnLongPress: Boolean = false, numberHint: Char? = null) = LetterGroupButton(if (capitalizationMode > 0) letters.uppercase(Locale.US) else letters, numberHint, if (numberOnLongPress) ({ commitCurrent(); currentInputConnection?.commitText((numberHint ?: action).toString(), 1) }) else null).apply { setOnClickListener { press(action) } }
    ...
    private fun createAlphabeticView(): View = keyboardContainer().apply {
        ...
        last.addView(key(capsLabel(), 'U'), LinearLayout.LayoutParams(0, dp(29), 1.2f).apply { setMargins(dp(2),dp(2),dp(2),dp(2)) })
    ...
    private fun press(key: Char) {
        ...
        when (key) {
            ...
            'U' -> { capitalizationMode = (capitalizationMode + 1) % 3; setInputView(onCreateInputView()) }
            in 'a'..'z' -> { lastDirectKeyAt = System.currentTimeMillis(); currentInputConnection?.commitText(outputWord(key.toString()), 1); lastCommitWasWord = true; consumeSingleCaps(); updateStandardPredictions() }
        }
    }
    private fun manualPress(key: Char) {
        val letters = groups[key] ?: return; handler.removeCallbacks(commitManual)
        if (lastKey == key) tapIndex = (tapIndex + 1) % letters.length else { if (pendingManual.isNotEmpty()) { currentInputConnection?.commitText(pendingManual, 1); consumeSingleCaps() }; tapIndex = 0 }
        val letter = letters[tapIndex].toString()
        pendingManual = outputWord(letter); lastKey = key; handler.postDelayed(commitManual, 700); refresh()
    }
    private fun toggleKeyboardMode() { handler.removeCallbacks(commitManual); pattern = ""; clearWordSuggestions(); if (pendingManual.isNotEmpty()) { currentInputConnection?.commitText(pendingManual, 1); consumeSingleCaps() }; pendingManual = ""; manual = !manual; lastKey = null; setInputView(onCreateInputView()) }
    private fun shouldAutoCapitalize(): Boolean { if (emailMode || !keyboardSettings.getBoolean("auto_capitalize", true)) return false; val before = currentInputConnection?.getTextBeforeCursor(256, 0)?.toString().orEmpty(); val last = before.trimEnd().lastOrNull(); return last == null || last == '.' || last == '!' || last == '?' }
    private fun outputWord(word: String) = when { capitalizationMode == 2 -> word.uppercase(Locale.US); capitalizationMode == 1 || shouldAutoCapitalize() -> word.replaceFirstChar { it.uppercase(Locale.US) }; else -> word }
    private fun consumeSingleCaps() { if (capitalizationMode == 1) { capitalizationMode = 0; setInputView(onCreateInputView()) } }
    private fun insertSpaceOrPeriod() { if (emailMode) { currentInputConnection?.commitText(" ", 1); return }; val before = currentInputConnection?.getTextBeforeCursor(256, 0)?.toString().orEmpty(); val beforeSpace = before.dropLastWhile { it.isWhitespace() }.lastOrNull(); if (keyboardSettings.getBoolean("double_space_period", true) && before.endsWith(" ") && beforeSpace?.isLetterOrDigit() == true) { currentInputConnection?.deleteSurroundingText(1, 0); currentInputConnection?.commitText(". ", 1) } else currentInputConnection?.commitText(" ", 1) }
    private fun commitCurrent() { if (manual) { handler.removeCallbacks(commitManual); if (pendingManual.isNotEmpty()) { currentInputConnection?.commitText(pendingManual, 1); consumeSingleCaps() }; pendingManual = "" } else if (pattern.isNotEmpty()) { val word = matches().firstOrNull(); if (word != null) { currentInputConnection?.commitText(outputWord(word), 1); recordUsage(word); lastCommitWasWord = true; consumeSingleCaps() }; pattern = ""; clearWordSuggestions() }; refresh() }
    ...
    private fun usageScore(word: String) = commonWordRank.getOrDefault(word, 0) + wordUsage.getInt(word, 0) * 1_000
    private fun recordUsage(word: String) { wordUsage.edit().putInt(word, wordUsage.getInt(word, 0) + 1).apply() }
    private fun clearWordSuggestions() { wordSuggestions = emptyList(); punctuationSuggestions = false; replacementBefore = 0; replacementAfter = 0; suppressWordSuggestionsUntil = System.currentTimeMillis() + 250 }
    private fun commitSuggestedWord(word: String) { val before = currentInputConnection?.getTextBeforeCursor(64, 0)?.toString().orEmpty(); if (lastCommitWasWord || before.lastOrNull()?.isLetterOrDigit() == true) currentInputConnection?.commitText(" ", 1); currentInputConnection?.commitText(outputWord(word), 1); val autoSpace = !emailMode && keyboardSettings.getBoolean("auto_space_suggestions", true); if (autoSpace) currentInputConnection?.commitText(" ", 1); recordUsage(word); lastCommitWasWord = !autoSpace; consumeSingleCaps() }
    private fun replaceCurrentWord(word: String) { val before = replacementBefore; val after = replacementAfter; clearWordSuggestions(); currentInputConnection?.deleteSurroundingText(before, after); currentInputConnection?.commitText(outputWord(word), 1); recordUsage(word); lastCommitWasWord = true; consumeSingleCaps(); refresh() }
    ...
    private fun commitStandardPrediction(word: String) { val replace = standardReplaceBefore; standardPrediction = emptyList(); standardReplaceBefore = 0; currentInputConnection?.deleteSurroundingText(replace, 0); currentInputConnection?.commitText(outputWord(word), 1); recordUsage(word); lastCommitWasWord = true; consumeSingleCaps(); refresh() }
    ...
    private fun commitPunctuation(mark: String) { val before = currentInputConnection?.getTextBeforeCursor(64, 0)?.toString().orEmpty(); if (before.endsWith(" ")) currentInputConnection?.deleteSurroundingText(1, 0); currentInputConnection?.commitText(mark + if (emailMode) "" else " ", 1); lastCommitWasWord = false; standardPrediction = emptyList(); refresh() }
    private fun refresh() { val punctuationMarks = listOf(".", ",", "?", "!", "'", "\"", ":", ";", "-", "(", ")", "@", "#", "&", "/"); val words = if (punctuationSuggestions) punctuationMarks else if (!manual && pattern.isNotEmpty()) matches() else if (manual && standardPrediction.isNotEmpty()) standardPrediction else wordSuggestions; status?.text = when { punctuationSuggestions -> "Punctuation suggestions — scroll and tap"; !manual && pattern.isNotEmpty() -> "T9 suggestions"; manual && standardPrediction.isNotEmpty() -> "Predictions"; wordSuggestions.isNotEmpty() -> "Same T9 code"; else -> "Suggestions" }; suggestions?.removeAllViews(); words.forEach { word -> suggestions?.addView(Button(this).apply { text = if (punctuationSuggestions) word else outputWord(word); textSize = 15f; isAllCaps = false; isSingleLine = true; ellipsize = TextUtils.TruncateAt.END; minimumWidth = 0; minWidth = 0; setPadding(dp(14), 0, dp(14), 0); setOnClickListener { if (punctuationSuggestions) { punctuationSuggestions = false; commitCurrent(); commitPunctuation(word) } else if (pattern.isNotEmpty()) { commitSuggestedWord(word); pattern = ""; clearWordSuggestions(); refresh() } else if (manual && standardPrediction.isNotEmpty()) commitStandardPrediction(word) else replaceCurrentWord(word) }; if (!punctuationSuggestions) setOnLongClickListener { hiddenWords.edit().putBoolean(word, true).apply(); wordSuggestions = wordSuggestions.filterNot { it == word }; standardPrediction = standardPrediction.filterNot { it == word }; Toast.makeText(this@T9InputMethodService, "Removed $word", Toast.LENGTH_SHORT).show(); refresh(); true }; styleButton(this) }, LinearLayout.LayoutParams(-2, -1).apply { setMargins(dp(2), 0, dp(2), 0) }) } }
}
